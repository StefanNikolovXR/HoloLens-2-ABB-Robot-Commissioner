...
VertexOutput vert (VertexInput v) {
	...
	#ifdef SECTOR
		half angStart = UNITY_ACCESS_INSTANCED_PROP(Props, _AngleStart);
		half angEnd = UNITY_ACCESS_INSTANCED_PROP(Props, _AngleEnd);
...
// minX, minY, maxX, maxY
inline half4 GetBounds( bool centerAlign, half alignOffset, half angDelta, half innerFrac, half outerFrac ) {
	half yMinRootRef = -(1-outerFrac); // padded for LAA
	...
	half tipMaxY = dir.y;
		...
		if(roundCaps){
			half uvRingRadius = (1-innerFrac)*0.5;
			...
			half2 dirSclInner = dir*innerFrac;
