...
public class SmartRecord : MonoBehaviour
{
    ...
    public BeginRecordingRobot recorder;
    public RPMAxisMonitoring rpm;
    public bool smartrecordon = false, endsmartrecordon = false, changed = false;
    public LeanMaterialColor recordon;
    public ColorSwitcher blinker;

    void Update()
    {

        if (smartrecordon == true)
        {
            if (rpm.j1vel != 0 || rpm.j2vel != 0 || rpm.j3vel != 0 || rpm.j4vel != 0 || rpm.j5vel != 0 || rpm.j6vel != 0)
                recorder.RecordRobotMotion();
                smartrecordon = false;
                changed = true;
                 blinker.enable = true;
        }

        if(endsmartrecordon == true) { 

        if (rpm.j1vel == 0 && rpm.j2vel == 0 && rpm.j3vel == 0 && rpm.j4vel == 0 && rpm.j5vel == 0 && rpm.j6vel == 0)
        {
                recorder.ReplayGoLive();
                blinker.enable = false;
                IsUntoggled();
                endsmartrecordon = false;
        }
        }
    }
    ...
    public void ToggleSmartRecord()
    {
        ...
        else
        {
          ...
          smartrecordon = true;
          endsmartrecordon = true;
          recordon.BeginThisTransition();
        }
    }
