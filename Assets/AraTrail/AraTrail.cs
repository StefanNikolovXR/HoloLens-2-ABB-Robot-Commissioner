...
namespace Ara{
    ...
    public class AraTrail : MonoBehaviour {
        ...
        private void EmissionStep(float time)
        {
            ...
            if (accumTime >= timeInterval) {
                ...
                if (emit) {
                    ...
                    Vector3 position = worldToTrail.MultiplyPoint3x4(transform.position);
        ...
        public void EmitPoint(Vector3 position)
        {
            ...
            points.Add(new Point(position, initialVelocity + velocity * inertia, transform.right, transform.forward, initialColor, initialThickness, texcoord, time));
        }
        ...
        private void UpdateSegmentMesh(int start, int end, Vector3 localCamPosition)
        {
            ...
            if (trail.Count > 1)
            {
                ...
                for (int i = trail.Count - 1; i >= 0; --i)
                {
                    ...
                    float normalizedLife = Mathf.Clamp01(1 - data[i].life / time);
