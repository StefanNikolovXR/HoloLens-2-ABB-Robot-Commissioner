...
public partial class TransformUtil : EditorWindow
{
	...
	{
		DrawGrid(gridOffset, gridLines, 
		         Vector3.right * grid.x, Vector3.forward * grid.z,
	...
	{
		DrawGrid(gridOffset, gridLines, 
		         Vector3.forward * grid.x, Vector3.up * grid.y,
	...
	{
		DrawGrid(gridOffset, gridLines, 
		         Vector3.up * grid.y, Vector3.left * grid.z,
	...
	/// A <see cref="Color"/>
	/// </param>
	private static void DrawGrid(Vector3 anOffset, 
	                             int aNumberOfGridLines, 
	                             Vector3 aDirection1, 
	                             Vector3 aDirection2,
	                             Color aGridColor, 
	                             Color aDirection1CenterColor, 
	                             Color aDirection2CenterColor)
	{
		...
		for (int x = -s; x <= s; x++)
		{
			for (int y = -s; y <= s; y++)
			{
				...
				Vector3 pos = anOffset + 
					((x * aDirection1) + (y * aDirection2));
				if (x < s)
